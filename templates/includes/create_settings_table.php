 ...
 * 
 * This script creates the system_settings table for storing global application settings
 */
...
try {
    ...
    if (!$tableExists) {
        ...
        $stmt = $conn->prepare("DESCRIBE system_settings");
        $stmt->execute();
        $columns = $stmt->fetchAll(PDO::FETCH_COLUMN);
        
        if (!in_array('is_public', $columns)) {
            $conn->exec("ALTER TABLE system_settings ADD COLUMN is_public BOOLEAN NOT NULL DEFAULT 1 AFTER data_type");
            echo "Added is_public column to system_settings table.\n";
        }
        
        if (!in_array('description', $columns)) {
            $conn->exec("ALTER TABLE system_settings ADD COLUMN description TEXT NULL AFTER is_public");
            echo "Added description column to system_settings table.\n";
        }
    }
